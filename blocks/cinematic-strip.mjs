...
export function cinematicStrip(items, { width = 896, height = 190, gap = 19, idPrefix = "strip" } = {}) {
  ...
  const cards = items.map((item, i) => {
    ...
    <a href="${xml(item.url)}">
      ...
      <rect x="${x}" y="0" width="${cardW}" height="${height}" rx="14" fill="url(#${idPrefix}-fade)" clip-path="url(#${clipId})"/>
      <text x="${x + 18}" y="${height - 24}" font-size="18" font-weight="800" fill="#ffffff">${xml(item.title)}<tspan dx="9" font-size="12.5" font-weight="500" fill="${palette.text}" opacity="0.75">${item.year}</tspan></text>
