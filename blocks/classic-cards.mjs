...
// horizontal card per item (small cover left, title + meta right). 878 x 220 for three items.
...
export function classicCards(items, { cardWidth = 258, gap = 18, margin = 34, idPrefix = "classic" } = {}) {
  ...
  const cards = items.map((item, i) => {
    ...
    <a href="${safeHref(item.url)}">
      ...
      <text x="${x + 112}" y="${y + 93}" font-size="12" fill="${palette.muted}">${xml(item.year)} / ${xml(item.episodes)} episodes</text>
    </a>`;
  }).join("\n");
